...
let waitingPlayer = null; // Lưu người đang chờ

io.on("connection", (socket) => {
  socket.on("find_match", (username) => {
    if (!waitingPlayer) {
      waitingPlayer = { id: socket.id, name: username };
      socket.emit("match_waiting");
    } else {
      // Ghép với người đang chờ
      const roomName = "rps_" + waitingPlayer.id + "_" + socket.id;
      socket.join(roomName);
      io.to(waitingPlayer.id).socketsJoin(roomName);
      
      io.to(roomName).emit("match_found", {
        players: [waitingPlayer.name, username],
        room: roomName
      });
...
io.on('connection', (socket) => {
    socket.on('rpsChoice', ({ room, player, choice }) => {
        if (!rpsGames[room]) rpsGames[room] = {};
        rpsGames[room][player] = choice;

        // Khi có 2 người chơi trong phòng
        const players = Object.keys(rpsGames[room]);
        if (players.length === 2) {
            const [p1, p2] = players;
            const c1 = rpsGames[room][p1];
            const c2 = rpsGames[room][p2];

            const result = getRPSResult(c1, c2);
            io.to(room).emit('rpsResult', {
                playerChoice: c1,
                opponentChoice: c2,
                result
            });
            delete rpsGames[room]; // reset game
        }
    });
});
