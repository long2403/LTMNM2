...
function sendRPSChoice(choice) {
    if (!joinedRoom) {
        alert("Vui lòng tham gia phòng trước khi chơi!");
        return;
    }
    socket.emit("rpsChoice", { room: joinedRoom, player: sender, choice });
    document.getElementById("rpsResult").innerHTML = "<div>Đang chờ đối thủ...</div>";
...
socket.on("rpsResult", (data) => {
    ...
    resultDiv.innerHTML = `
        <div>Bạn chọn: ${data.playerChoice}</div>
        <div>Đối thủ chọn: ${data.opponentChoice}</div>
        <div><b>Kết quả: ${data.result}</b></div>
    `;
});
